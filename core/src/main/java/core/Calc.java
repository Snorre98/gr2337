...
public class Calc {
    ...
    public double performOperation(UnaryOperator<Double> op) throws IllegalStateException {
        if (getOperandCount() < 1) {
            throw new IllegalStateException("Must be elements in the stack");
        }
        var topOperand = popOperand();
        var result = op.apply(topOperand);
        pushOperand(result);
        return result;
    }
    ...
    public void swap() {
        ...
        var operandTop = popOperand();
        var operandSecondTop = popOperand();

        pushOperand(operandTop);
        pushOperand(operandSecondTop);
    }
